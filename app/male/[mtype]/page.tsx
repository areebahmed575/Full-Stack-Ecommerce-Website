...
async function fetchAllProductsData() {
  let res = await fetch(`https://${process.env.NEXT_PUBLIC_SANITY_PROJECT_ID}.api.sanity.io/v2023-06-08/data/query/production?query=*%5B_type%20%3D%3D%20%22testing%22%20%26%26%20productTypes%5B0%5D%3D%3D%20%22Male%22%5D%0A`, {
    next: {
      revalidate: 60
    }
  });
...
const Male = async ({ params }: { params: { mtype: string } }) => {
  ...
  if (params.mtype !== "Male") {
    let orginalSortedDataOfParams = res.result.filter((items: IProduct) => items.productTypes[1] === params.mtype)
